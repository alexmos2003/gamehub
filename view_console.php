...
if ($BASE === '' || $BASE === '\\') { $BASE = '/'; }
function img_url($value){ /* identic ca în calculatoare */ 
    global $BASE; $v=trim((string)$value);
...
<div class="grid-container">
  <div class="grid-x grid-padding-x">
    <div class="large-12 cell">
      <h2 style="font-size:42px;margin:10px 0 20px;">Toate consolele</h2>
      </br>

      <?php
      $db2 = new mysqli("localhost", "root", "", "gamehub");
      if ($db2->connect_error) die("Connection failed: " . $db2->connect_error);
      $sqlv = "SELECT id, name, description, price, image_url FROM tabel_console";
      ...
      <a href="admin_console.php" class="button">Înapoi</a>
      <?php $db2->close(); ?>
    </div>
  </div>
</div>
